...
When calling the Pachinko `add_note` tool for archived SEC filings:

- Always use `note_body_file_path` with the Markdown file created by the bundled converter.
- Never send the rendered filing in the `note_body` parameter.

This Codex project provides the Pachinko MCP server for the user's Sync EDGAR work.
