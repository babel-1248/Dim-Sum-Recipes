#!/usr/bin/env python3
"""Cap an EDGAR keep list while leaving overflow eligible for the next run."""
...
def manifest_entries(manifest):
    return [
        (filing["accession"], filing.get("filed") or manifest.get("since"))
        for filing in manifest.get("documents", [])
    ]
