...
When calling the Pachinko `add_note` tool for archived PubMed papers:

- Always use `note_body_file_path` with the Markdown file created by the bundled converter.
- Never send the full rendered paper in the `note_body` parameter.

This Codex project provides the Pachinko MCP server for the user's Sync PubMed work.
