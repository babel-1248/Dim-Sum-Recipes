...
CLI:
  ...
  python3 feedstate.py pending FEED --index INDEX [--batch-size N]
...
def pending(feed, index_path, batch_size, excluded=()):
    ...
    ids = [entry.get("pmid") or entry.get("id") for entry in entries]
    if any(not item_id for item_id in ids):
        raise SystemExit(f"{index_path}: every entry must have pmid or id")
    if len(ids) != len(set(ids)):
        raise SystemExit(f"{index_path}: duplicate paper ids make completion ambiguous")
    ...
    def compact(entry):
        return {
            "pmid": entry.get("pmid") or entry.get("id"),
